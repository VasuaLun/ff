...
begin
	...
	.th7 {text-align:left; width:200px} .c7 {text-align:left; width:200px}
	.th8 {width:100%; text-align:left;} .c8 {width:100%; text-align:left;}
	.th9 {width:200px; text-align:center;} .c9 {width:200px; text-align:center;}
	...
	for rec in
	(
		select distinct D.RN, D.CODE
		from Z_DEPFIN D, Z_USERS U
		where (U.DEPFIN = D.RN and U.RN = nUSERS) or (ZGET_ROLE = 0)
		order by CODE
	)
	loop
	htp.p('<div style="background:white; border:1px solid #e1e1e1;margin-bottom:5px;">
			<div class="version-header" style="">'||rec.CODE||'</div>
			<div class="version-list">
			<table cellpadding="0" cellspacing="0">
			<thead>
			<tr>
			<th class="th th7">Версия</th>
			<th class="th th8">Примечания</th>
			<th class="th th9">Статус</th>
			</tr>
			</thead>
			<tbody>');
		for ver in
		(
			select V.RN as RN,
			V.CODE as CODE,
			V.DEFULT_SIGN as DEFULT_SIGN,
			V.NOTES as NOTE
			from Z_DEPFIN_VERS V
			where V.DEPFIN_RN = rec.RN
			order by V.CODE
		)
		loop
			nWARNING := '';
			if ver.RN = :P1_DEPFIN_VERSION then
				sLight := 'style ="background:rgba(15, 220, 31, 0.39);"';
			else
				sLight := 'style =""';
			end if;
			...
			begin
				select GRV.GRBS_VERSION, GRB.JURPERS_RN into nVERSION, nGRBS
				from Z_DEPFIN_GRBS GRB, Z_DEPFIN_VERS_GRBS GRV, Z_VERSIONS V
				where GRV.GRBS_VERSION = V.RN and GRB.JURPERS_RN = V.JUR_PERS and GRB.DEPFIN_RN = rec.RN and rownum = 1
				order by GRB.JURPERS_RN desc;
				exception
			...
			if nVERSION is NULL then nWARNING := 'Нет привязанных версий ГРБС. '; end if;

			if nGRBS is NULL then nWARNING := concat(nWARNING, 'Нет привязанных ГРБС.'); end if;

			if (nGRBS is not NULL and nVERSION is not NULL) then nWARNING := ver.NOTE; end if;

			htp.p('<tr '||case when (ver.DEFULT_SIGN != 0) and (nGRBS is not NULL or nVERSION is not NULL) then 'class="light"' when (nGRBS is NULL or nVERSION is NULL) then 'class="red"' end||'>
								<td class="c c7" '|| sLight ||'><div class="c7"><a href="f?p=&APP_ID.:1:&SESSION.::&DEBUG.:1,7:P1_DEPFIN,P1_DEPFIN_VERSION,P1_JURPERS,P1_VERSION:'||rec.RN||','||ver.RN||','||nGRBS||','||nVERSION||'" class="link_code">'||ver.CODE||'</a></div></td>
						<td class="c c8" '|| sLight ||'><div class="c8">'|| nWARNING ||'</div></td>
						<td class="c c9" '|| sLight ||'><div class="c9">'||case when ver.DEFULT_SIGN != 0 then 'ОСНОВНАЯ' end||'</div></td>
				  </tr>');
		end loop;
		htp.p('</tbody></table></div></div>');
	end loop;
