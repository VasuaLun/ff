...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    (
        select c001,
            to_number(c002) as ORGRN,
            c003,
            to_number(c004) as POST_FACT,
            to_number(c005) as EXP_DIFF,
            to_number(c006) as IND_DIFF,
            to_number(c007) as NORMAIV,
            n001 as PLANLIMSUM,
            n002 as PLANOUTSUM,
            n003 as IND_CACL,
            n004 as IND_VAL,
        ...
        case when pSORT = 2  then PLANLIMSUM end asc,
        case when pSORT = -2 then PLANLIMSUM end desc,
        case when pSORT = 3  then PLANOUTSUM end asc,
        case when pSORT = -3 then PLANOUTSUM end desc,
        case when pSORT = 4  then EXP_DIFF   end asc,
        case when pSORT = -4 then EXP_DIFF   end desc,
        case when pSORT = 5  then IND_CACL   end asc,
        case when pSORT = -5 then IND_CACL   end desc,
        case when pSORT = 6  then IND_VAL    end asc,
        case when pSORT = -6 then IND_VAL    end desc,
        ...
        case when pSORT = 11  then FOT_PERC  end asc,
