...
begin
    begin
        ...
        if dMAINSOGLDATE is not null then
                sMAINDATE := '"' ||to_char(dMAINSOGLDATE, 'dd') ||'" ' || LOWER(F_GET_MONTH (to_char(dMAINSOGLDATE, 'mm'),1)) || ' ' || to_char(dMAINSOGLDATE,'yyyy')||' г.';
        else
       ...
       order by TGP.PERIOD_NUM
        ...
        begin -- Body
            ...
            APKG_XLSREP.OPEN_SHEET(nPROTECTED => 1, sORIENTATION => 'landscape', sSHEETNAME => 'Приложение к доп.соглашению', sPASSWORD => ZF_GET_REP_PASS);
            APKG_XLSREP.ADD_RUNTITLE(sFOOTERLEFT => sFOOTERLEFT,  sHEADERRIGTH => sHEADERRIGTH);
            APKG_XLSREP.OPEN_TABLE();

            APKG_XLSREP.ADD_COLUMN(nWIDTH => 120);
            APKG_XLSREP.ADD_COLUMN(nWIDTH => 100, nCOUNT => 6);
            APKG_XLSREP.ADD_COLUMN(nWIDTH => 120);
            APKG_XLSREP.ADD_COLUMN(nWIDTH => 120, nCOUNT => RGRAPH.COUNT);

            nValueRow := 'Приложение
                          к доп.соглашению № '||sDOPSOGLNUMB||' от '||sDOPDATE||' к Соглашению о порядке и условиях предоставления государственному бюджетному и
            ...
            APKG_XLSREP.ADD_CELL(sSTYLE => 'top_left_str', sCELLDATA => sORGPERSPOSITION || ' ' ||sORGSIGNORGNAME, sCELLTYPE => 'String', nMERGEACROSS => 2, nSKIPINDEX => 8);
            APKG_XLSREP.ADD_ROW (nHEIGHT => 50);
            ...
            APKG_XLSREP.ADD_CELL(sSTYLE => 'bottom_left_str', sCELLDATA => lpad('/ ', 40- length(sORGDIRECTOR), '_') ||sORGDIRECTOR, sCELLTYPE => 'String', nMERGEACROSS => 2, nSKIPINDEX => 8);
            APKG_XLSREP.ADD_ROW (nHEIGHT => 50);
