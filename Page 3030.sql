...
begin
	...
	loop
        ...
        sDOWNLOAD     := '<td class="c2"><div class="c2">'||replace(rec.ADD_MIMETYPE, 'application/', '.')||'</div></td>';

        sNAME     := '<td class="c2"><div class="c2">'||rec.ADD_FILENAME||'</div></td>';

        sDOWNLOAD := '<td class="c3"><div class="c3"><a href="'||APEX_UTIL.PREPARE_URL('f?p='||:APP_ID||':3030:'||:APP_SESSION||':rDownload:::P3030_RN:'||rec.ATTACH_ID)||'"  title="Скачать файл отчета"><img width="25" height ="25" src="/i/ico/xls.gif"/></a></div></td>';
