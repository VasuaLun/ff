...
begin
    ...
    select null PARENT_ROW,  P.ORGRN, O.CODE SHORT_NAME,
           ...
           K.RN KOSGURN, K.CODE KOSGUCODE,
           0 PAYSUM,
		  	   case when P.AGRDATE is not null
    ...
    loop
        if rec.PARENT_ROW is not null then
		    ...
		    if (nvl(nESUM_OUTC,0) + nvl(nESUM_REST,0)) != 0
           or nvl(rec.PAYSUM,0) != 0
           or nvl(rec.ZKPSUM_REQUISITS,0) != 0
           or nvl(rec.ZKPSUM_NO_REQUISITS,0) != 0 then
