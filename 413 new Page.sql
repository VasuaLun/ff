...
begin
   ...
   select NEW_TRANS_GRAPH
    ...
    .th2{width: 200px;text-align:center;}  .c2 {width: 200px; word-wrap: break-word; text-align:left;}
    ...
    begin
      select SUM(case when nvl(P.REST_SIGN,0) = 1 then - G.SUMMA else G.SUMMA end)
       ...
       where G.ORGRN = pORGRN
         ...
         and G.EXPDIR    = :P1420_EXPDIR
		 and (G.FUND_RN = :P1420_FUND or :P1420_FUND is null)
		 and (G.KBK_RN = :P1420_KBK or :P1420_KBK is null)
		 and (G.FOTYPE2 = :P1420_FOTYPE2 or :P1420_FOTYPE2 is null)
		 and (G.PERIOD_RN = :P1420_PERIOD or :P1420_PERIOD is null);
    ...
    select RTYPE, sum(SUMMA) SUMMA
     ...
     where PRN = pORGRN
       ...
       and TOTAL_SIGN = 0
	   and (KBK = :P1420_KBK or :P1420_KBK is null)
	   and (FUND = :P1420_FUND or :P1420_FUND is null)
	   and (FOTYPE2 = :P1420_FOTYPE2 or :P1420_FOTYPE2 is null)
     group by RTYPE
    )
    loop
        if rec.RTYPE = 0 then
    ...
    <thead>
      ...
      <tr>
        <th class="header th2"  style="border-left:none"><div class="th2" style="text-align:center">Период</div></th>
        <th class="header th0"  ><div class="th0" style="text-align:center">Срок</div></th>
		<th class="header th6"  ><div class="th6" style="text-align:center">Статья затрат</div></th>
	...
	for MAIN in
	(
	  select G.*, L.NOTE sSTATUS, P.REST_SIGN, P.PERIOD_NUM || ' - ' || P.CODE PERIOD, E.CODE EXPMATART
		from Z_TRANSFER_GRAPH G, Z_TRANSFER_GRAPH_PERIODS P, Z_LOV L, ZV_KBKALL KBK, Z_FUNDS F, Z_EXPMAT E
	   where G.ORGRN     = pORGRN
		 and G.VERSION   = pVERSION
		 and G.EXPDIR    = :P1420_EXPDIR
		 and G.STATUS    = L.NUM (+)
		 ...
		 and (G.PERIOD_RN = :P1420_PERIOD or :P1420_PERIOD is null)
		 and G.EXPMAT = E.RN (+)
	   order by G.EXPDIR, lpad(KBK.SFULLKBKCODE,50), lpad(F.SUBCODE,50), lpad(F.CODE,10), P.PERIOD_NUM, G.RN, G.PERIOD_NAME
	)
	loop
		sBTN  := 'btn';
		select count(*) into nHISTCOUNT from Z_TRANSFER_GRAPH_HISTORY where PARENT_ROW = MAIN.RN;
		...
		if MAIN.FUND_RN is null then
			if (nPrevKBK is null) or (MAIN.KBK_RN <> nPrevKBK) then
				...
				htp.p('<td class="c0" colspan="2"><div class="c0" style ="text-align:right; color:#aa2b15;">ПО КБК:<b> '||sKBK_NAME||'</b></div></td>
				   '||sSUMMA||'
			...
			if (nPrevFUND is null or (nvl(MAIN.FUND_RN,0) <> nvl(nPrevFUND,0))) or (nPrevKBK is null or (nvl(MAIN.KBK_RN,0) <> nvl(nPrevKBK,0))) then
				...
				--<td class="c4" ><div class="c4" style ="text-align:right; color:#aa2b15;"></div></td>
				htp.p('<td class="c0" colspan="2"><div class="c0" style ="text-align:right; color:#aa2b15;">' ||case when nvl(sRECIPIENT_LIST,0) = 'Терапевты' then '<a class="link_code" href="'||APEX_UTIL.PREPARE_URL('f?p='||:APP_ID||':465:'||:APP_SESSION||'::::P465_FUND,P465_ORGRN,P465_KBK,P1420_SELECT_ROW:'||MAIN.FUND_RN ||','|| pOrgRn ||','|| MAIN.KBK_RN ||','|| MAIN.RN)||'">По ЦС: '||sFUND_NAME||'('||sKBK_NAME||'</a>'
																												 when nvl(sRECIPIENT_LIST,0) = 'Доступная среда' then '<a class="link_code" href="'||APEX_UTIL.PREPARE_URL('f?p='||:APP_ID||':464:'||:APP_SESSION||'::::P464_FUND,P464_ORGRN,P464_KBK,P1420_SELECT_ROW:'||MAIN.FUND_RN ||','|| pOrgRn ||','|| MAIN.KBK_RN ||','|| MAIN.RN)||'">По ЦС: '||sFUND_NAME||'('||sKBK_NAME||'</a>'
                                                                                                                 when nvl(sRECIPIENT_LIST,0) = 'Цели расходов' then '<a class="link_code" href="'||APEX_UTIL.PREPARE_URL('f?p='||:APP_ID||':510:'||:APP_SESSION||'::::P510_FUND,P510_ORGRN,P510_KBK,P1420_SELECT_ROW:'||MAIN.FUND_RN ||','|| pOrgRn ||','|| MAIN.KBK_RN ||','|| MAIN.RN)||'">По ЦС: '||sFUND_NAME||'('||sKBK_NAME||'</a>'
																												 else 'По ЦС>: '||sFUND_NAME||'('||sKBK_NAME end ||'</div></td>
				   '||sSUMMA||'
				   <td class="c4" style="border-left:1px solid #ccc"><div class="c4" style ="text-align:right; color:#aa2b15;"></div></td>
				   <td class="c8" style="border-left:1px solid #ccc"><div class="c8" style ="text-align:right; color:#aa2b15;"></div></td>
				   <td class="c9" style="border-left:1px solid #ccc"><div class="c9" style ="text-align:right; color:#aa2b15;"></div></td>
		...
		if nvl(nHIST,0) = 1 then
			sSUMMA     := '<td class="c1" style="border-left:1px solid #ccc">
						   <div class="c1"><input disabled placeholder="0,00" type="text" value="'||LTRIM(to_char(MAIN.SUMMA,'999G999G999G999G999G990D00'),' ')||'" class="in_txt2 decimal"
								 onfocus="selecter(this,''row_'||MAIN.rn||''')" onblur="save_data('||MAIN.rn||',this.value, ''SUMMA'', this);"/>'
								 || case when nREDACTIONS = 1 then '<span class="'||sBTN||'" style="float:right"   onclick="location.href='''||APEX_UTIL.PREPARE_URL('javascript:modalWin2('||MAIN.RN||','||MAIN.EXPDIR||');')||'''">+</span>' end ||
						   '</div>
						   </td>';
		else
			sSUMMA := '<td class="c1" style="border-left:1px solid #ccc"><div class="c1">
					<input '|| sDISABLED ||' type="text" value="'||LTRIM(to_char(nvl(MAIN.SUMMA,0),'999G999G999G999G999G990D00'),' ')||'"
					class="in_txt2 decimal" onfocus="validate(this) ;selecter(this,''row_'||MAIN.RN||''') "
					onblur="save_data('||MAIN.RN||',this.value, ''SUMMA'', this);"/></div></td>';
		end if;

        htp.p('<td class="c2" ><div class="c2"></div>'||MAIN.PERIOD||'</td>
        	   <td class="c0" style="border-left:1px solid #ccc"><div class="c0">
        			<input '|| sDISABLED ||' type="text" value="'||MAIN.PERIOD_NAME||'"
        			class="in_txt" onfocus="validate(this) ;selecter(this,''row_'||MAIN.RN||''') "
        			onblur="save_data('||MAIN.RN||',this.value, ''PERIOD_NAME'', this);"/></div></td>
        	   <td class="c6" style="border-left:1px solid #ccc"><div class="c6" >'||MAIN.EXPMATART||'</div></td>
        	   <td class="c4" style="border-left:1px solid #ccc"><div class="c4" ></div></td>
        	   '||sSUMMA||'
        	   <td class="c4" style="border-left:1px solid #ccc"><div class="c4" ></div></td>
        	   <td class="c8" style="border-left:1px solid #ccc"><div class="c8"><a href="f?p=101:413:&APP_SESSION.:rADDROW:NO::P413_RN:'||main.RN||'"><img style="width:12px" src="/i/FNDADD11.gif" title="Добавить строку" /></a></div></td>
        	   '||
        	   case when nvl(MAIN.HAND_ADD,0) = 1 then

        			'<td class="c9" style="border-left:1px solid #ccc"><div class="c9"><a href="f?p=101:413:&APP_SESSION.:rDELROW:NO::P413_DELRN:'||main.RN||'"><img style="width:12px" src="/i/menu/remove_16x16.gif" title="Удалить строку" /></a></div></td>'
        	        when nvl(MAIN.HAND_ADD,0) = 0 then

        		'<td class="c9" style="border-left:1px solid #ccc;"><div class="c9" ></div></td>' end
        	   ||'
        	   </tr>');
  ...
  htp.p('<script>
    ...
    var el = document.getElementById("row_"+$v("P1420_SELECT_ROW"));
   if (el!==null){
